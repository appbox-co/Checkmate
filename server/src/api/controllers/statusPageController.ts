...
class StatusPageController implements IStatusPageController {
	...
	getStatusPageByUrl = catchAsync(async (req: Request, res: Response) => {
		...
		const { range, monitorId, incidentPage } = getStatusPageQueryValidation.parse(req.query);
		...
		const data = await this.statusPageService.getPublicStatusPagePayload(statusPage, req.user?.teamId, range, { monitorId, incidentPage });
	...
	resolveStatusPageByDomain = catchAsync(async (req: Request, res: Response) => {
		const { range, monitorId, incidentPage } = resolveStatusPageQueryValidation.parse(req.query);
